...
try {
    $stmt = $db->prepare("SELECT purpose_of_visit, health_history, medicine, quantity, remarks FROM admin_history WHERE patient_id = ? ORDER BY created_at DESC LIMIT 1");
    $stmt->execute([$patientId]);
...
try {
    $stmt = $db->prepare("SELECT date, time_in, time_out, medicine, quantity, health_history, purpose_of_visit, remarks FROM admin_history WHERE patient_id = ? ORDER BY created_at DESC");
    $stmt->execute([$patientId]);
