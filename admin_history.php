            ...
            if (move_uploaded_file($file_tmp, $inputFileName, )) {
                try {
                    ...
                    foreach ($sheetData as $row) {
                        ...
                        $query = "INSERT INTO admin_history (patient_id, name, date, time_in, time_out, medicine, quantity, health_history, purpose_of_visit, remarks, created_at)
                                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
                        $stmt = $db->prepare($query);
                        $stmt->execute([$patient_id, $name, $date, $time_in, $time_out, $medicine, $quantity, $health_history, $purpose_of_visit, $remarks]);
                        $insertCount++;
                    }
